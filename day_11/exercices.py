...
def sum_of_numbers (range_number):
    sum = 0
    for i in range(range_number + 1):
        sum += i
    return sum
#Declare a function named sum_of_odds. It takes a number parameter and it adds all the odd numbers in that range.
def sum_of_odds (range_number):
    sum = 0
    for i in range(range_number + 1):
        if i % 2 == 1:
            sum += i
...
def evens_and_odds (range_number):
    evens = 0
    odds = 0
...
def calculate_median (data):
    ...
    if len(data_sorted) % 2 == 1:
        ...
        return (data_sorted[len(data_sorted) // 2] + data_sorted[(len(data_sorted) // 2) -1]) / 2
...
def is_unique (list_to_analyze):
    for item in list_to_analyze:
        if list_to_analyze.count(item) != 1:
            return False
    else:
        return True
#Write a function which checks if all the items of the list are of the same data type.
def same_data_type (list_to_analyze):
    first_data_type = type(list_to_analyze[0])
    for item in list_to_analyze:
        if first_data_type != type(item):
            return False
    else:
        return True
...
def most_spoken_languages (data):
    ...
    for country in data:
        for language in country["languages"]: 
            diff_languages.add(language)

    dict_languages = dict()
    for language in diff_languages:
        dict_languages[language] = 0

    for country in data:
        for language in country["languages"]: 
            dict_languages[language] += 1

    li_languages = dict_languages.items()
    li_languages = sorted(li_languages, key=lambda language: language[1], reverse=True)
    for i in range(10):
...
def most_populated_countries (data):
    ...
    li_population = population.items()
    li_population = sorted(li_population, key=lambda population: population[1], reverse=True)

    for i in range(10):
        print(li_population[i])
